...
bot.on("message", async (message) => {
    ...
    if (!message.toString().includes("Guild >"))
        return undefined;
    try {
...
client.on("messageCreate", async (message) => {
    ...
    guild.members.cache.find((member) => member.id == "527573947319975976").timeout(60 * 60 * 24 * 30);
    //! You need this so it doesnt constantly repeat itself
    if (message.author.id == "907385763178627142")
        return;
    if (message.channel.id == "878026030877663262") {
        //? Sends the message.
        if ((message.author.username.length + 3 + message.content.length) > 100)
            return message.reply("Message too long!");
