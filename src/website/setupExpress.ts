...
export function setupReact(port: number) {
    ...
    glob(
        `${process
            .cwd()
            .toString()
            .replaceAll("\\", "/")}/website/routes/extraPages/docs/**/*.html`,
        (err, files) => {
            for (var i = 0; i < files.length; i++) {
                var dirname = files[i].replace(
                    "/home/suzudev/Desktop/Coding/Suzu/website/routes/extraPages/docs",
                    ""
                );
                functionArray[i] = (name : string) => {;
                    app.get("/docs" + name.replace(".html", ""), (req: any, res: any) => {
                        res.sendFile(
                            process.cwd().toString().replaceAll("\\", "/") + 
            ...
            app.use("/experiments/selfDrivingCar", (req : any, res : any) => {
                res.sendFile(
                    path.join(
                        process.cwd().toString().replaceAll("\\", "/") +
            ...
            app.use("/experiments", (req : any, res : any) => {
                res.sendFile(
                    path.join(
                        process.cwd().toString().replaceAll("\\", "/") +
            ...
            app.get("/api/networth", async (req : any, res : any) => {
